...
class PneumoniaDetectorApp(tk.Frame):
    def __init__(self, master=None):
        ...
        self.model_lr_4features = self.load_model(models_dir, 'logistic_regression_model.pkl')
        self.model_lr_1feature = self.load_model(models_dir, 'logreg-v3.pkl')
        self.cnn_model_path = os.path.join(models_dir, 'my_model-v4-bestOne.h5')
    ...
    def extract_features(self, num_features):
        ...
        if num_features == 1:
            # Extraction de 1 caractéristique (par exemple, gabor_mean)
            gabor_feats = self.extractor.compute_gabor(image)
            gabor_mean = np.mean(gabor_feats)
            feature_vector = np.array([gabor_mean]).reshape(1, -1)
